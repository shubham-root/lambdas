...
pub struct Position(OrderedFloat<f32>, OrderedFloat<f32>);
...
impl Bounds {
    ...
    fn min_x(&self) -> OrderedFloat<f32> {
        let (min, _) = self.min_max.unwrap();
        min.0
    }
    fn min_y(&self) -> OrderedFloat<f32> {
        let (min, _) = self.min_max.unwrap();
        min.1
    }

    fn max_x(&self) -> OrderedFloat<f32> {
        let (_, max) = self.min_max.unwrap();
        max.0
    }
    fn max_y(&self) -> OrderedFloat<f32> {
        let (_, max) = self.min_max.unwrap();
        max.1
    }
}
...
pub struct Degree(pub OrderedFloat<f32>);

#[derive(Copy, Clone, Debug)]
pub struct Radiant(pub OrderedFloat<f32>);
...
impl Into<Degree> for OrderedFloat<f32> {
    fn into(self) -> Degree {
        Degree(self)
    }
}
...
#[derive(Copy, Clone, Debug)]
pub struct Distance(OrderedFloat<f32>);

impl Into<Distance> for OrderedFloat<f32> {
...
impl Eq for Canvas {} // The empty implementation signifies all fields must match
...
impl Canvas {
    pub fn new() -> Canvas {
        ...
        let init_state = TurtleState {
            ...
            angle: Degree(OrderedFloat(0.0)), // points upwards
    ...
    #[inline]
    fn direction(&self, distance: Distance) -> (OrderedFloat<f32>, OrderedFloat<f32>) {
        let state = self.current_state();
        let rad: Radiant = state.angle.into();
        let (sin, cos) = rad.0.sin_cos();
        let dx = -OrderedFloat(sin) * distance.0;
        let dy = OrderedFloat(cos) * distance.0;
        (dx, dy)
    }
...
"#,
    ...
    pub fn save_svg<W: Write>(&self, wr: &mut W) -> io::Result<()> {
        ...
        for path in self.paths.iter() {
            if let Some((head, tail)) = path.split_first() {
                // XXX
                let head = Position(head.0, OrderedFloat(-1.0) * head.1);

                write!(wr, r#"<path d="M{} {}"#, head.0, head.1)?;
                for pos in tail {
                    let pos = Position(pos.0, OrderedFloat(-1.0) * pos.1);
